...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        _mongoClient = this._client.getServiceClient(LocalMongoDbService.clientFactory);
        setupLogin();
    }
    ...
    private void addItem(final String text) {
        ...
        try {
            getItemsCollection().insertOne(doc);
            refreshList();
        } catch (Exception ex) {
            Log.e(TAG, "Error adding item", ex);
        }
    }
    ...
    private void clearChecked() {
        ...
        try {
            getItemsCollection().deleteMany(query);
            refreshList();
        } catch (Exception ex) {
            Log.e(TAG, "Error clearing checked items", ex);
        }
    }

    private MongoCollection getItemsCollection() {
    ...
    private List<TodoItem> convertDocsToTodo(final List<Document> documents) {
        final List<TodoItem> items = new ArrayList<TodoItem>(documents.size());
    ...
    private void refreshList() {
        ...
        FindIterable cursor = getItemsCollection().find(filter).limit(100);
